...
import { X, Save, FileText, Copy, Check, AlertTriangle, Zap, Droplets, ChevronRight } from "lucide-react";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ padding: "8px 12px", borderRadius: 8, background: "var(--blue-50)", border: "1px solid var(--blue-100)", fontSize: 12.5, color: "var(--blue-700)", display: "flex", alignItems: "center", gap: 7 }}>
        ...
        Chỉ số nhập tháng <strong>{period}</strong> sẽ dùng để tính điện/nước trực tiếp cho hoá đơn tháng <strong>{period}</strong>
      </div>
          ...
          <>
            <div style={{ background: "var(--vn-surface)", border: BD, borderRadius: 10, overflow: "hidden" }}>
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: 13 }}>
                <thead>
                  <tr>{["Phòng", "Khách thuê", `Điện tháng ${prevPeriod(period)}`, `Nhập điện tháng ${period}`, ...(isPerMeter ? [`Nước ${prevPeriod(period)}`, `Nhập nước ${period}`] : []), "HĐ"].map(TH)}</tr>
                </thead>
                <tbody>
                  {rows.map((row, i) => {
                    ...
                    return (
                      <tr key={row.room_id} style={{ background: hasInvoice ? "var(--green-50)" : row.reading_id ? "var(--blue-50)" : "transparent" }}>
                        <td style={{ padding: "10px 12px", borderBottom: bd }}><span style={{ fontWeight: 700, fontFamily: "var(--font-geist-mono)", fontSize: 12.5 }}>P.{row.room_number}</span></td>
                        ...
                        <td style={{ padding: "10px 12px", borderBottom: bd, fontVariantNumeric: "tabular-nums" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                            <Zap size={11} color="var(--amber-400)" />
                            {row.prev_elec_curr ? <span style={{ color: "var(--vn-text-2)" }}>{fmtN(row.prev_elec_curr)}</span>
                              : isNew ? <span style={{ fontSize: 10.5, background: "var(--amber-50)", color: "var(--amber-700)", padding: "1px 6px", borderRadius: 999 }}>Mới</span>
                        ...
                        <td style={{ padding: "7px 12px", borderBottom: bd }}>
                          {hasInvoice
                            ...
                            : <input type="number" min={0} step="0.01" value={inp.elec_curr}
                                ...
                                style={{ width: 100, height: 30, padding: "0 8px", border: BD, borderRadius: 6, fontSize: 13, background: "#fff", outline: "none", fontVariantNumeric: "tabular-nums" }} />
                          }
                        </td>
                        {isPerMeter && (
                          <>
                            ...
                            <td style={{ padding: "7px 12px", borderBottom: bd }}>
                              {hasInvoice
                                ...
                                : <input type="number" min={0} step="0.01" value={inp.water_curr}
                                    ...
                                    placeholder="Nhập..."
                                    style={{ width: 82, height: 30, padding: "0 8px", border: BD, borderRadius: 6, fontSize: 13, background: "#fff", outline: "none", fontVariantNumeric: "tabular-nums" }} />
                              }
                            </td>
                          </>
                        )}
                        <td style={{ padding: "10px 12px", borderBottom: bd }}>
                          {row.invoice_status ? <StatusBadge status={row.invoice_status} />
                            : row.reading_id ? <span style={{ fontSize: 11, color: "var(--blue-600)" }}>Có chỉ số</span>
                            : <span style={{ fontSize: 11, color: "var(--vn-text-3)" }}>Chưa nhập</span>}
                        </td>
                      </tr>
                    );
                  })}
            ...
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              <button onClick={handleSave} disabled={saving || changed.length === 0} style={{ display: "inline-flex", alignItems: "center", gap: 6, height: 34, padding: "0 16px", borderRadius: 8, background: changed.length > 0 ? "var(--blue-600)" : "var(--slate-100)", color: changed.length > 0 ? "#fff" : "var(--vn-text-3)", fontSize: 13, fontWeight: 500, border: "none", cursor: changed.length > 0 ? "pointer" : "default", opacity: saving ? 0.7 : 1 }}>
...
// ── Invoice Tab ───────────────────────────────────────────────────
function InvoiceTab({ propertyId, period }: { propertyId: string; period: string }) {
  const { getToken } = useAuth();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ padding: "8px 12px", borderRadius: 8, background: "var(--blue-50)", border: "1px solid var(--blue-100)", fontSize: 12.5, color: "var(--blue-700)", display: "flex", alignItems: "center", gap: 7 }}>
        ...
        Hoá đơn tháng <strong>{period}</strong> · Điện/nước tính từ chỉ số tháng <strong>{period}</strong> (nhập ở tab bên trái)
      </div>
                    ...
                    return (
                      <tr key={row.contract_id} style={{ background: rowBg, opacity: isLocked ? 0.7 : 1, cursor: isLocked ? "default" : "pointer" }}
                        ...
                        <td style={{ padding: "10px 12px", borderBottom: bd, verticalAlign: "middle", fontVariantNumeric: "tabular-nums" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                            {!row.has_reading && <AlertTriangle size={11} color="var(--amber-500)" />}
                            <span style={{ color: !row.has_reading ? "var(--vn-text-3)" : "var(--vn-text)" }}>{fmtM(row.elec_amount)}</span>
                          </div>
                        </td>
                        <td style={{ padding: "10px 12px", borderBottom: bd, verticalAlign: "middle", fontVariantNumeric: "tabular-nums", color: "var(--vn-text-2)" }}>{fmtM(row.water_amount)}</td>
                        ...
                        <td style={{ padding: "10px 12px", borderBottom: bd, verticalAlign: "middle" }} onClick={e => e.stopPropagation()}>
                          <div style={{ display: "flex", gap: 4 }}>
                            ...
                            {row.invoice_id && (
                              <button onClick={() => setDrawerInvoiceId(row.invoice_id!)} style={{ display: "inline-flex", alignItems: "center", gap: 3, height: 24, padding: "0 7px", borderRadius: 5, border: BD, background: "var(--vn-surface)", color: "var(--blue-600)", fontSize: 11, cursor: "pointer", fontWeight: 500 }}>
      ...
      <InvoiceDrawer invoiceId={drawerInvoiceId} onClose={() => setDrawerInvoiceId(null)}
        onUpdate={(updated: Invoice) => setRows(prev => prev.map(r => r.invoice_id === updated.id ? { ...r, invoice_status: updated.status as InvoiceStatus } : r))} />
    </div>
  );
}
...
export function BillingModal({ propertyId, property, open, onClose }: BillingModalProps) {
  ...
  return (
    <>
      {/* Backdrop */}
      <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 500, background: "rgba(0,0,0,0.3)", opacity: open ? 1 : 0, pointerEvents: open ? "auto" : "none", transition: "opacity 0.2s" }} />

      {/* Panel — slides up from bottom */}
      <div style={{
        position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 501,
        background: "var(--vn-bg, #fff)",
        ...
        <div style={{ display: "flex", justifyContent: "center", padding: "10px 0 6px" }}>
          <div style={{ width: 40, height: 4, borderRadius: 2, background: "var(--vn-border)" }} />
        </div>

        {/* Header */}
        <div style={{ padding: "0 24px 12px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: BD, flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, color: "var(--vn-text)", letterSpacing: "-0.01em" }}>
              Tính tiền · {property?.name}
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <input type="month" value={period} onChange={e => setPeriod(e.target.value)} style={{ height: 32, padding: "0 10px", borderRadius: 7, border: BD, background: "var(--vn-surface)", fontSize: 13, color: "var(--vn-text)", outline: "none", fontFamily: "var(--font-geist-mono)" }} />
        ...
        {/* Tabs */}
        <div style={{ display: "flex", borderBottom: BD, padding: "0 24px", flexShrink: 0 }}>
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)} style={{ height: 36, padding: "0 16px", border: "none", background: "none", cursor: "pointer", fontSize: 13.5, fontWeight: tab === t.key ? 600 : 400, color: tab === t.key ? "var(--blue-600)" : "var(--vn-text-2)", borderBottom: tab === t.key ? "2px solid var(--blue-600)" : "2px solid transparent", marginBottom: -1 }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px 24px" }}>
          {tab === "readings" && <ReadingTab propertyId={propertyId} period={period} property={property} />}
          {tab === "invoices" && <InvoiceTab propertyId={propertyId} period={period} />}
        </div>
      </div>
    </>
  );
