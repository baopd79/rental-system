...
def upgrade() -> None:
    ...
    op.create_unique_constraint('uq_surcharge_property_name', 'surcharge_template', ['property_id', 'name'])


def downgrade() -> None:
    """Downgrade schema."""
    op.drop_constraint('uq_surcharge_property_name', 'surcharge_template', type_='unique')
    op.alter_column(
        'surcharge_template',
        'name',
