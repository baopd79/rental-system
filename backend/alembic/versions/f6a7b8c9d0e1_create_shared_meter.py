...
def upgrade() -> None:
    ...
    with op.get_context().autocommit_block():
        op.execute(
            sa.text("ALTER TYPE invoiceitemtype ADD VALUE IF NOT EXISTS 'shared_elec'")
        )
    ...
    op.create_table(
        ...
        sa.Column(
            "property_id", sa.Integer(), sa.ForeignKey("property.id"), nullable=False
        ),
    ...
    op.create_index(
        "ix_shared_meter_reading_meter_id", "shared_meter_reading", ["shared_meter_id"]
    )
