...
from app.schemas.contract import ContractCreate, ContractRead, ContractUpdate, ContractReadWithRoom, ContractEventRead, ContractListItem

router = APIRouter(tags=["contracts"])


@router.get("/contracts", response_model=list[ContractListItem])
async def list_all_contracts(clerk_user_id: CurrentUserDep, service: ContractServiceDep):
...
async def list_tenant_contracts(tenant_id: int, clerk_user_id: CurrentUserDep, service: ContractServiceDep):
...
async def get_contract_events(contract_id: int, clerk_user_id: CurrentUserDep, service: ContractServiceDep):
