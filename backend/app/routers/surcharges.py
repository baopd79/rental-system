...
async def list_surcharges(
    property_id: int, clerk_user_id: CurrentUserDep, service: SurchargeServiceDep
):
    return await service.list_surcharges(property_id, clerk_user_id)


@router.post(
    "/properties/{property_id}/surcharges",
    response_model=SurchargeRead,
    status_code=201,
)
async def create_surcharge(
    property_id: int,
    body: SurchargeCreate,
...
async def delete_surcharge(
    surcharge_id: int, clerk_user_id: CurrentUserDep, service: SurchargeServiceDep
):
