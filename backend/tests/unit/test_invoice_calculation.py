...
from app.services.invoice_service import _build_items
...
def test_rent_item_uses_agreed_rent():
    ...
    prop = make_property()
    items = _build_items(contract, None, [], prop, prop.default_elec_rate)
...
def test_elec_normal():
    ...
    items = _build_items(contract, reading, [], prop, prop.default_elec_rate)
...
def test_elec_uses_effective_rate():
    ...
    items = _build_items(contract, reading, [], prop, Decimal("4000"))
    elec = items_by_type(items, InvoiceItemType.electricity)[0]
...
def test_surcharge_per_room():
    ...
    items = _build_items(contract, None, surcharges, prop, prop.default_elec_rate)
...
def test_total_sum():
    ...
    surcharges = [SurchargeTemplate(id=1, property_id=1, name="Wifi",
                                    calc_type=SurchargeCalcType.per_room, amount=Decimal("100000"))]
    items = _build_items(contract, reading, surcharges, prop, prop.default_elec_rate)
