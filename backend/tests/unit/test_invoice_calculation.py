...
def test_surcharge_per_room():
    ...
    items = _build_items(
        contract, None, surcharges, prop, prop.default_elec_rate, "2026-05"
    )
...
def test_total_sum():
    ...
    items = _build_items(
        contract, reading, surcharges, prop, prop.default_elec_rate, "2026-05"
    )
...
def test_prorate_factor_full_month():
    assert _prorate_factor(date(2026, 1, 1), date(2026, 12, 31), "2026-05") == Decimal(
        "1"
    )
...
def make_partial_contract(
    start: date, end: date, agreed_rent: str = "3000000", num_people: int = 2
) -> Contract:
    return Contract(
        id=1,
        room_id=1,
...
def test_rent_prorated_for_partial_month():
    ...
    contract = make_partial_contract(
        date(2026, 5, 16), date(2026, 12, 31), agreed_rent="3100000"
    )
    prop = make_property()
    items = _build_items(contract, None, [], prop, prop.default_elec_rate, "2026-05")
...
def test_water_per_person_prorated():
    contract = make_partial_contract(
        date(2026, 5, 16), date(2026, 12, 31), num_people=2
    )
