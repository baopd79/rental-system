...
@pytest.mark.asyncio
async def test_new_tenant_elec_prev_is_not_inherited_from_old_tenant():
    ...
    async with AsyncClient(
        ...
        # Tenant 1: starts Mar (initial reading 2026-03), billing entered for 2026-03
        contract1 = await setup_contract(
            client, USER_A, room["id"], "2026-03-01", "2026-03-31"
        )
        await batch_readings(
            client,
            USER_A,
            prop["id"],
            "2026-03",
            [{"room_id": room["id"], "elec_curr": "1500"}],
        )
        await client.put(
            f"/api/v1/contracts/{contract1['id']}/end", headers=auth_headers(USER_A)
        )

        # Tenant 2: starts Apr (initial reading 2026-04 = 1000), billing entered for 2026-04
        await setup_contract(client, USER_A, room["id"], "2026-04-01", "2026-12-31")
        r = await batch_readings(
            client,
            USER_A,
            prop["id"],
            "2026-04",
            [{"room_id": room["id"], "elec_curr": "1200"}],
        )

    assert r.status_code == 200
    row = next(x for x in r.json() if x["room_id"] == room["id"])
    # elec_prev = move-in reading (1000), NOT inherited from tenant 1's 1500
    assert float(row["elec_prev"]) == 1000
    assert float(row["elec_curr"]) == 1200
...
async def test_month_skip_within_same_contract_returns_400():
    ...
    async with AsyncClient(
        ...
        prop, room = await setup_property_room(client, USER_A)
        await setup_contract(client, USER_A, room["id"], "2026-03-01")

        # Enter March reading (same as move-in month → UPDATE path, no skip guard)
        await batch_readings(
            client,
            USER_A,
            prop["id"],
            "2026-03",
            [{"room_id": room["id"], "elec_curr": "1300"}],
        )
...
async def test_edit_locked_when_downstream_reading_exists():
    ...
    async with AsyncClient(
        ...
        prop, room = await setup_property_room(client, USER_A)
        await setup_contract(client, USER_A, room["id"], "2026-04-01")
        ...
        await batch_readings(
            ...
            prop["id"],
            "2026-04",
            [{"room_id": room["id"], "elec_curr": "1400"}],
        )
        await batch_readings(
            client,
            USER_A,
        ...
        r = await batch_readings(
            ...
            prop["id"],
            "2026-04",
            [{"room_id": room["id"], "elec_curr": "1410"}],
        )
...
async def test_edit_locked_when_invoice_exists():
    ...
    async with AsyncClient(
        ...
        prop, room = await setup_property_room(client, USER_A)
        contract = await setup_contract(client, USER_A, room["id"], "2026-05-01")
        ...
        r = await batch_readings(
            ...
            prop["id"],
            "2026-05",
            [{"room_id": room["id"], "elec_curr": "1510"}],
        )
...
async def test_batch_readings_forbidden_for_other_user():
    async with AsyncClient(
        ...
        r = await client.post(
            ...
            json={
                "period": "2026-05",
                "readings": [{"room_id": room["id"], "elec_curr": "500"}],
            },
